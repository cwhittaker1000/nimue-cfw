...
vaccine_simplified_ <- R6::R6Class(
  "odin_model",
  cloneable = FALSE,
  ...
  private = list(
    ...
    interpolate_t = NULL,
    cfuns = list(
      rhs_dde = "vaccine_simplified_rhs_dde",
      rhs_desolve = "vaccine_simplified_rhs_desolve",
      initmod_desolve = "vaccine_simplified_initmod_desolve",
      output_dde = "vaccine_simplified_output_dde"),
    dll = "nimue",
    user = c("beta_set", "D_0", "E_0", "gamma_E", "gamma_ICase",
             ...
             "N_prioritisation_steps", "N_vaccine", "prob_death_hosp",
             "prob_hosp", "R_0", "rel_infectiousness",
             "rel_infectiousness_vaccinated", "S_0", "tt_beta", "tt_matrix",
             "tt_vaccine", "tt_vaccine_efficacy_infection",
    ...
    registration = function() {
      if (FALSE) {
        .C("vaccine_simplified_rhs_dde", package = "nimue")
        .C("vaccine_simplified_rhs_desolve", package = "nimue")
        .C("vaccine_simplified_initmod_desolve", package = "nimue")
        .C("vaccine_simplified_output_dde", package = "nimue")
      }
    },

    ## This only does something in delay models
    set_initial = function(t, y, use_dde) {
      .Call("vaccine_simplified_set_initial", private$ptr, t, y, use_dde,
            PACKAGE= "nimue")
    },

    update_metadata = function() {
      meta <- .Call("vaccine_simplified_metadata", private$ptr,
                    PACKAGE = "nimue")
      private$variable_order <- meta$variable_order
      private$output_order <- meta$output_order
      private$n_out <- meta$n_out
  ...
  public = list(
    initialize = function(..., user = list(...), use_dde = FALSE,
                          unused_user_action = NULL) {
      private$odin <- asNamespace("odin")
      private$ptr <- .Call("vaccine_simplified_create", user, PACKAGE = "nimue")
      self$set_user(user = user, unused_user_action = unused_user_action)
      private$use_dde <- use_dde
      private$update_metadata()
    },
    ...
    ir = function() {
      path_ir <- system.file("odin/vaccine_simplified.json", mustWork = TRUE,
                             package = "nimue")
      json <- readLines(path_ir)
      class(json) <- "json"
      json
    ...
    set_user = function(..., user = list(...), unused_user_action = NULL) {
      private$odin$support_check_user(user, private$user, unused_user_action)
      .Call("vaccine_simplified_set_user", private$ptr, user, PACKAGE = "nimue")
      private$update_metadata()
    },
    ...
    ## time in.
    initial = function(t) {
      .Call("vaccine_simplified_initial_conditions", private$ptr, t, PACKAGE = "nimue")
    },

    rhs = function(t, y) {
      .Call("vaccine_simplified_rhs_r", private$ptr, t, y, PACKAGE = "nimue")
    },
    ...
    contents = function() {
      .Call("vaccine_simplified_contents", private$ptr, PACKAGE = "nimue")
    },
...
vaccine_simplified <- function(..., user = list(...), use_dde = FALSE,
                     unused_user_action = NULL) {
  asNamespace("odin")$deprecated_constructor_call("vaccine_simplified")
  vaccine_simplified_$new(user = user, use_dde = use_dde,
                unused_user_action = unused_user_action)
}
class(vaccine_simplified) <- "odin_generator"
attr(vaccine_simplified, "generator") <- vaccine_simplified_
